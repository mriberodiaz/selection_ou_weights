...
def server_update(model, server_optimizer, server_state, weights_delta,num_participants, global_norm_mean, global_norm_var):
  ...
  if server_state.round_num<1:
    ...
    pred = tf.nest.map_structure(lambda g: clip_ops.clip_by_norm(g,1.0), pred)
...
def create_client_update_fn():
  ...
  def client_update(model,
    ...
    for batch in dataset:
      ...
      grads = tf.nest.map_structure(lambda g: clip_ops.clip_by_norm(g,1.0), grads)
    ...
    client_delta_norm = tf.linalg.global_norm(weights_delta)
    weights_delta, client_transmitted = estimation_utils.zero_weights_randomly(
      structure=weights_delta, 
      replace_structure=predicted_delta, 
      prob_transmit = client_transmit, 
    )
...
def build_fed_avg_process(
  ...
  @tff.tf_computation(model_input_type, model_weights_type, round_num_type, predicted_delta_type, tf.bool)
  def client_update_fn(tf_dataset, initial_model_weights, round_num, predicted_delta, client_transmit):
    ...
    return client_update(model_fn(), tf_dataset, initial_model_weights,
                         client_optimizer, predicted_delta, client_transmit,client_weight_fn)
  ...
  @tff.federated_computation(
      ...
      tff.FederatedType(tf.bool, tff.CLIENTS, all_equal=False))
  def run_one_round(server_state, federated_dataset, federated_client_transmit):
    ...
    client_outputs = tff.federated_map(
        client_update_fn,
        (federated_dataset, client_model, client_round_num,client_predicted_delta, federated_client_transmit))
